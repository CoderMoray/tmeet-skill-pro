...
> **辅助脚本**：`scripts/check-conflict.sh`

当用户要求批量创建会议（「下周一到周五每天上午9点站会」「明天开始连续三天面试」）时：

1. AI 解析用户输入 → 生成 `[{date, start, end, subject}]` JSON 数组
2. 管道传入脚本检测冲突：
   ```bash
   echo '[...]' | scripts/check-conflict.sh
   ```
3. 脚本输出带 `conflict` / `conflict_with` 标记的结果，展示给用户确认
4. 确认后逐条执行 `tmeet meeting create`

---

## 受邀者继承

> **辅助脚本**：`scripts/transfer-invitees.sh`

将历史会议的受邀人复制到新会议：

1. `tmeet meeting list` / `list-ended` → AI 按主题/时间语义匹配源会议（多候选时列出让用户选）
2. 展示受邀人列表和来源会议信息，**二次确认**
3. 执行转移：
   ```bash
   scripts/transfer-invitees.sh <源 meeting_id> <目标 meeting_id>
   ```
   - 脚本会自动处理分页，提取所有 open_id 后一次调用 `invitees-add`
   - 若新会议未创建，先 `meeting create` 拿到 meeting_id 再执行转移
